...
Game.RESOLVE_COMBAT = new Action().init({
  ...
  doAction: function(gameId, userId) {
    ...
    if (attacker.attacking === true) {
      ...
      var index;
      for (index = 0; index < supporters.length; ++index) {
...
Game.STEAL_CARD = new Action().init({
  ...
  callback: function(data, extraData) {
    ...
    winner.cards.push({card: cardId, card_state: 0})
...
Game.END_TURN = new Action().init({
  _id: 10,
  name: "END_TURN",
  desc: "End the turn after a free response.",
  callback: function(data, extraData) {
  },
  canDoAction: function(gameId) {
    return true;
...
Game.TRADE = new Action().init({
  _id: 4,
  name: "TRADE",
  desc: "Trade with another player",
  callback: function(data, extraData) {
    // target trade player gets to reject or chooses card to trade back
  },
  canDoAction: function(gameId) {
    return true;
  },
