...
Game.PLAY_PROFESSION = new Action().init({
  ...
  callback: function(data, extraData) {
    getProfById(data.prof).onUse(data.gameId, data.user);
  },
  ...
  doAction: function(gameId, userId) {
    ...
    if (this.canDoAction(gameId, userId)) {
      ...
      if (game.state.meta.hasOwnProperty("start_turn")) { 
        newMeta = game.state.meta;
      } else {
        newMeta = {
...
Game.FREE_RESPONSE = new Action().init({
  ...
  doAction: function(gameId, userId) {
    ...
    var nextPlayerTurn = getRealTurn(game.players, getPlayer(game.players, userId).turn + 1);
    var nextPlayer = getPlayerByTurn(game.players, nextPlayerTurn);

    // if next player to respond was the initiater
    if (nextPlayerTurn === game.state.meta.start_turn) { 
      switch(game.state.meta.type) {
        case Game.PLAY_PROFESSION._id:
          // next player's turn is now over
          nextPlayerTurn = (nextPlayerTurn + 1) % game.players.length;
          nextPlayer = getPlayerByTurn(game.players, nextPlayerTurn);
          return {
            action: Game.TURN_START._id,
            user: nextPlayer._id,
            wait_on: nextPlayer._id,
...
Game.DECLARE_VICTORY = new Action().init({
  ...
  callback: function(data, extraData) {
    ...
    for(var i = 0; i < nominatedPlayerIds.length; i++) {
      ...
      if (DEBUG) {
        console.log("num_victory_cards: " + num_victory_cards);
        console.log(num_victory_cards);
      }
