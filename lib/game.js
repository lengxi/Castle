    ...
    if (hasDestinedWinner === false &&
      ...
      if (!((attacker.prof === Professions.SWORDSMAN._id &&
        ...
        supporters = _.reject(game.players, function(p) {
          return p.attacking || p.defending || p.supports === -1 || p.hypnotized;
        });
  ...
  doAction: function(gameId, userId, extraData) {
    ...
    switch(card) {
      ...
      case Cards.MONOCLE._id:
        var monocleTarget = (trader._id === game.state.user) 
            ? game.state.meta.trade_target : game.state.user;
        var monocleTargetPlayer = getPlayer(players, monocleTarget);
        Games.update({_id: gameId, "players._id": trader._id}, 
            {$set: {"players.$.monocle_target_assoc": monocleTargetPlayer.assoc}});
        break;
      case Cards.PRIVILEGE._id:
        var privilegeTarget = (trader._id === game.state.user) 
            ? game.state.meta.trade_target : game.state.user;
        var privilegeTargetPlayer = getPlayer(players, privilegeTarget);
        Games.update({_id: gameId, "players._id": trader._id}, 
            {$set: {"players.$.privilege_target_cards": privilegeTarget.cards}});
        break;
