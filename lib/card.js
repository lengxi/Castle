...
Cards.GOBLET = new Card().init({
  _id: 1,
  name: "Goblet (OoTL)",
  desc: "The Order of True Lies can proclaim victory if it holds at least 3 Goblets.  Pay attention to the Bag of Secrets - Goblet . It will become a Goblet as soon as the objects deck empties.",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.KEY = new Card().init({
  _id: 2,
  name: "Key (SoOS)",
  desc: "The Society of Open Secrets can proclaim victory if it holds at least 3 Keys.  Pay attention to the Bag of Secrets - Key . It will become a Key as soon as the objects deck empties.",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.BAG_KEY = new Card().init({
  _id: 3,
  name: "Bag of Secrets (key/SoOS)",
  desc: "Trade this card in to draw a card. You cannot trade this card for another Bag of Secrets. When deck runs out, this turns into a Key (Society of Open Secrets).",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.BAG_GOBLET = new Card().init({
  _id: 4,
  name: "Bag of Secrets (goblet/OoTL)",
  desc: "Trade this card in to draw a card. You cannot trade this card for another Bag of Secrets. When deck runs out, this turns into a Goblet (Order of True Lies).",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.BLACK_PEARL = new Card().init({
  _id: 5,
  name: "Black Pearl",
  desc: "You must always accept this object if offered in a trade. If you hold the Black Pearl you cannot declare victory under any circumstance. The other members of your Secret Society can still proclaim victory normally, and if the declaration is correct you also win!",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.COAT = new Card().init({
  _id: 6,
  name: "Coat",
  desc: "Trade this object and you can choose a new profession from those left over. You will receive it in an unplayed state. Your current profession is returned to the profession deck. Professions that can be used only once may come back into play and be used again.",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.ASTROLABE = new Card().init({
  _id: 13,
  name: "Astrolabe",
  desc: "Trade this object and you can choose a direction (right or left). All players must then pass one object at random to their neighbor in the chosen direction. Passing an object in this way does not count as a trade. Any trade abilities of passed objects are not activated. The object traded for the Astrolabe is activated normally even if it is then passed to a neighbor because of the effect of the Astrolabe.",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
...
Cards.TOME = new Card().init({
  _id: 16,
  name: "Tome",
  desc: "Trade this object and you can exchange your profession with your trading partner. These professions become unplayed.  Professions that can be used only once may be used again if they are exchanged thanks to the Tome.",
  canUse: function(gameId, user, target) {
    return false;
  },
  onUse: function(gameId, user, target) {
